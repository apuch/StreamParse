...
START_TEST ( createView ) {
    initData4();
    IS_OK(sp_buffer_init_view(&_b1, &_b, 8, 2 * 8));
    IS_INT_EQ(_b1.length, 2 * 8);
    IS_INT_EQ(_b1.index, 0);
    IS_P_EQ(_b1.data, _data4 + 1);

    IS_OK(sp_buffer_init_view(&_b1, &_b, 8, 3 * 8));
...
START_TEST (createViewInvalid ) {
    initData4();
    IS_ERR(sp_buffer_init_view(&_b1, &_b, 7, 16), E_ALIGNMENT);
    IS_ERR(sp_buffer_init_view(&_b1, &_b, 8, 15), E_ALIGNMENT);
    IS_ERR(sp_buffer_init_view(&_b1, NULL, 8, 2 * 18), E_ARGUMENT);
    IS_ERR(sp_buffer_init_view(NULL, &_b, 8, 2 * 8), E_ARGUMENT);
    IS_ERR(sp_buffer_init_view(&_b1, &_b, 4 * 8, 2 * 8), E_RANGE);
    IS_ERR(sp_buffer_init_view(&_b1, &_b, 8, 4 * 8), E_RANGE);
} END_TEST
